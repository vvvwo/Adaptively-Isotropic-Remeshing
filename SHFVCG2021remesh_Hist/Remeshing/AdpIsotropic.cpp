...
/*
void Hist_Draw(std::vector<double> data) {
...
public:
	...
	void MeshOptimization_init(string fileN, int adtParameter, vector<vector<double>> pointsi, vector<vector<int>> faceInfori) {
...
private:
	...
	void MeshOptimization_Apt_L_init(int classIndex) {
		...
		vector<double> cu_ave_t2 = cu_ave;
		std::sort(cu_ave_t2.begin(), cu_ave_t2.end());

		//Histogram-based analysis
		//double min_cu_ave2 = cu_ave_t2[0];
		//double max_cu_ave2 = cu_ave_t2[cu_ave_t2.size() - 1];
		//double unitStep2 = (max_cu_ave2 - min_cu_ave2) / 40.0;

		//vector<double> index2(40, 0);
		//for (int i = 0; i < 40; i++) {
			//double c_lower = min_cu_ave2 + unitStep2 * i;
			//double c_upper = min_cu_ave2 + unitStep2 * (i + 1);
			//for (int j = 0; j < cu_ave_t2.size(); j++) {
				//if (cu_ave_t2[j] <= c_upper && cu_ave_t2[j] >= c_lower) {
					//index2[i] = index2[i] + 1;
				//}
			//}
		//}
		...
		//Histogram-based analysis
		double min_cu_ave = cu_ave_t[0];
		double max_cu_ave = cu_ave_t[cu_ave_t.size()-1];
		double unitStep = (max_cu_ave - min_cu_ave) / 40.0;

		vector<double> index(40, 0);
		for (int i = 0; i < 40; i++) {
			double c_lower = min_cu_ave + unitStep * i;
			double c_upper = min_cu_ave + unitStep * (i + 1);
			for (int j = 0; j < cu_ave_t.size(); j++) {
				if (cu_ave_t[j] <= c_upper && cu_ave_t[j] >= c_lower) {
					index[i] = index[i] + 1;
				}
			}
		}
		...
		double c_lower_Mid = min_cu_ave + unitStep * hisMax;
		double c_upper_Mid = min_cu_ave + unitStep * (hisMax + 1);
		double c_base = (c_lower_Mid + c_upper_Mid) / 2;

		int indexC_Base = 0;
		for (int i = 1; i < cu_ave_t.size(); i++) {
			if (cu_ave_t[i - 1] <= c_base && c_base <= cu_ave_t[i]) {
				indexC_Base = i;
				break;			
			}			
		}

		int maxCu = cu_ave_t.size();
		int unitMax_indexC_Base = (maxCu - indexC_Base) / 5;
		double li1 = cu_ave_t[indexC_Base / 5 * 2];
		double li2 = cu_ave_t[indexC_Base / 5 * 4];
		double li3 = cu_ave_t[indexC_Base + unitMax_indexC_Base];
		double li4 = cu_ave_t[indexC_Base + unitMax_indexC_Base * 3];
		for (int i = 0; i < cu_ave.size(); i++) {
			if (cu_ave[i] < li1) {
				cu_ave[i] = 1.8 * L_ave;
			}
			...
			for (int i = 0; i < pointNeighbor.size(); i++) {
				...
				for (int j = 0; j < pointNeighbor[i].size(); j++) {
					...
					if (!MeshOptimization_Exist(b2, b2_list) && b1 < b2) {
						...
						double L_j1 = MeshOptimization_KdTreeSearch(b1);
						double L_j2 = MeshOptimization_KdTreeSearch(b2);
						if (L_j1 > L_j2) {
							L_j1 = L_j2;
						}
		...
		while (1) {
			...
			for (int i = 0; i < Collapse_List.size(); i++) {
				...
				for (int i = 0; i < pc_list.size(); i++) {
					...
					double L_j2 = MeshOptimization_KdTreeSearch(pN2);
					if (L_j1 > L_j2) {
						L_j1 = L_j2;
					}
	...
	bool MeshOptimization_If_Flip(int b1, int b2, int b3, int b4) {
		...
		double L_j1 = MeshOptimization_KdTreeSearch(b3);
		double L_j2 = MeshOptimization_KdTreeSearch(b4);
		if (L_j1 > L_j2) {
			L_j1 = L_j2;
		}
...
#pragma omp parallel for
	...
	void MeshOptimization_SaveCurvature(vector<double> dis) {

		string fileName2 = fileName + "_Curvature.txt";
		ofstream f1(fileName2);		
		f1 << dis.size() << endl;
